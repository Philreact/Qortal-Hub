...
const LAND_CHAT_BUBBLE_TTL_MS = 4000;
...
export function QortalLand({ groupId, groupName, myAddress }: QortalLandProps) {
  ...
  const [selectedDevPlacementId, setSelectedDevPlacementId] = useState(
    QORTAL_LAND_DEVELOPMENT_PARK_FLOOR_PLACEMENT_ID
  );
  ...
  useEffect(() => {
    ...
    const loadHistory = async () => {
      let history = await window.reticulumChat?.getMessageHistory?.(groupId, QORTAL_LAND_CHANNEL_ID, 40);
      if (!history) {
        history = await window.reticulumChat?.getHistory?.(groupId, QORTAL_LAND_CHANNEL_ID, 40);
      }
      history = history ?? [];
      ...
      const messages = history
        ...
        .map((event) => parseQortalLandChatEvent(event, sessionId))
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.reticulumChat?.onEvent?.(({ event }) => {
      ...
      const message = parseQortalLandChatEvent(payload, sessionId);
      if (!message) return;
      queuePrimaryNameLookups([payload.authorAddress]);
      const now = Date.now();
  ...
  }, [appendLandChatMessage, groupId, myAddress, queuePrimaryNameLookups, sessionId]);
