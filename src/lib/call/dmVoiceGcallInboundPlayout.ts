...
const GCALL_POST_BURST_LATENCY_SHED_EMERGENCY_MAX_FRAMES_PER_TICK = 8;
const GCALL_POST_BURST_LATENCY_SHED_EMERGENCY_OVER_TARGET_FRAMES = 12;
...
export function computePostBurstLatencyShedFrames(opts: {
  ...
  if (
    overTargetFrames >=
    GCALL_POST_BURST_LATENCY_SHED_EMERGENCY_OVER_TARGET_FRAMES
  ) {
    return Math.min(
      GCALL_POST_BURST_LATENCY_SHED_EMERGENCY_MAX_FRAMES_PER_TICK,
      overTargetFrames
    );
  }
  return Math.min(
