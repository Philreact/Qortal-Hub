...
const RETICULUM_LINK_CLOSE_RECENT_ACTIVITY_GRACE_MS = 30_000;
...
export class PresenceManager extends EventEmitter {
  ...
  noteReticulumOverlayLinkClosed(
    ...
    const keepActiveForRecentClose =
      wasVerified &&
      (reasonKey.includes('timeout') || reasonKey.includes('destination_closed')) &&
      typeof lastActivityAgeMs === 'number' &&
      Number.isFinite(lastActivityAgeMs) &&
      lastActivityAgeMs <= RETICULUM_LINK_CLOSE_RECENT_ACTIVITY_GRACE_MS;
    if (keepActiveForRecentClose) {
      loggerLog(
