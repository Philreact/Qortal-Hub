  ...
  it('kills a sidecar that emits malformed IPC without crashing Electron', async () => {
    const script = `
      ...
      readline.createInterface({ input: process.stdin }).on('line', (line) => {
        ...
        if (count++ === 0) process.stdout.write(JSON.stringify({version:2,type:'response',requestId:request.requestId,ok:true,result:{service:'qortal-private-transport',sidecarVersion:'0.9.1',protocolVersion:2,innerAlpn:'qortal-private/1',moqAlpn:'moqt-18'}}) + '\\n');
