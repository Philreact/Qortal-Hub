...
export class ReticulumBridge extends EventEmitter implements PresenceTransport {
  ...
  private async spawnAndHandshake(configDir: string): Promise<void> {
    ...
    loggerLog(
      `[ReticulumBridge] Launch env QORTAL_RNS_LINK_TRACE=${env.QORTAL_RNS_LINK_TRACE} QORTAL_RNS_LOCAL_TRACE=${env.QORTAL_RNS_LOCAL_TRACE ?? '0'} QORTAL_RNS_LOCAL_TRACE_FRAMES=${env.QORTAL_RNS_LOCAL_TRACE_FRAMES ?? '0'} RNS_RESOURCE_WINDOW=${env.RNS_RESOURCE_WINDOW} RNS_RESOURCE_WINDOW_MAX_FAST=${env.RNS_RESOURCE_WINDOW_MAX_FAST} RNS_RESOURCE_MAX_EFFICIENT_SIZE=${env.RNS_RESOURCE_MAX_EFFICIENT_SIZE} PYTHONPATH=${env.PYTHONPATH ?? ''}`
    );
