...
exports.loadPersistedAllowedDomainsAtStartup =
...
exports.registerLateReticulumBridgeRecovery =
...
const electron_window_state_1 = tslib_1.__importDefault(
...
function recordMainLoopStall(delayMs, nowMs = Date.now()) {
  ...
  (0, logger_1.warn)(
    `[GCall] target=reticulum-audio-ipc stage=main-event-loop-stall delay_ms=${Math.round(delayMs)} stall_count=${mainLoopStallCount} max_delay_ms=${Math.round(mainLoopStallMaxDelayMs)}`
  );
}
...
function getWalletStorageJsonPathSync() {
  return path.join(
    electron_2.app.getPath('appData'),
    'qortal-hub',
    'wallet-storage.json'
  );
}
...
let audioSurfaceBridgeState = (0,
audio_surface_ipc_1.buildDefaultAudioSurfaceBridgeStateLike)();
...
class ElectronCapacitorApp {
  constructor(capacitorFileConfig, trayMenuTemplate, appMenuBarMenuTemplate) {
    ...
    this.customScheme =
      ...
      'capacitor-electron';
    this.audioSurfaceScheme = (0,
    audio_window_policy_1.buildAudioSurfaceScheme)(this.customScheme);
    if (trayMenuTemplate) {
  ...
  async createAudioSurfaceWindow() {
    ...
    const preloadPath = (0, path_1.join)(
      electron_2.app.getAppPath(),
      'build',
      'src',
    ...
    const window = new electron_2.BrowserWindow({
      ...
      webPreferences: {
        ...
        preload: preloadPath,
        additionalArguments: [
          `--window-role=${audio_window_policy_1.AUDIO_SURFACE_WINDOW_ROLE}`,
        ],
      },
    });
    ...
    const targetUrl = (0, audio_window_policy_1.buildAudioSurfaceUrl)(
    ...
    window.webContents.on('did-finish-load', () => {
      ...
      void window.webContents
        ...
        .catch((error) => {
          (0, logger_1.warn)(
            '[GCall:audio-surface] runtime isolation probe failed',
            {
              webContentsId,
  ...
  async init(p2pBootstrapSeeds) {
    await (0, app_protocol_1.registerStaticAppProtocol)(
      electron_2.session.defaultSession,
      this.customScheme,
      (0, path_1.join)(electron_2.app.getAppPath(), 'app')
    );
    await (0, app_protocol_1.registerStaticAppProtocol)(
      electron_2.session.defaultSession,
      this.audioSurfaceScheme,
    ...
    this.audioSurfaceHttpsOrigin = await (0,
    audio_surface_https_1.ensureAudioSurfaceHttpsServer)(
      (0, path_1.join)(electron_2.app.getAppPath(), 'app')
    );
    const icon = electron_2.nativeImage.createFromPath(
      (0, path_1.join)(
        electron_2.app.getAppPath(),
        'assets',
    ...
    if (this.CapacitorFileConfig.electron?.trayIconAndMenuEnabled) {
      ...
      if (process.platform !== 'darwin') {
        ...
        this.TrayIcon.setContextMenu(
          electron_2.Menu.buildFromTemplate(this.TrayMenuTemplate)
        );
      }
    }
    // Setup the main manu bar at the top of our window.
    electron_2.Menu.setApplicationMenu(
      electron_2.Menu.buildFromTemplate(this.AppMenuBarMenuTemplate)
    );
    // If the splashscreen is enabled, show it first while the main window loads then switch it out for the main window, or just load the main window from the start.
    if (this.CapacitorFileConfig.electron?.splashScreenEnabled) {
      this.SplashScreen = new electron_1.CapacitorSplashScreen({
        imageFilePath: (0, path_1.join)(
          electron_2.app.getAppPath(),
          'assets',
    ...
    this.MainWindow.webContents.on('dom-ready', () => {
      ...
      setTimeout(() => {
        ...
        electron_1.CapElectronEventEmitter.emit(
...
function setupContentSecurityPolicy(customScheme) {
  electron_2.session.defaultSession.webRequest.onHeadersReceived(
    (details, callback) => {
  ...
  `
      ...
      Object.assign(
        responseHeaders,
        (0, audio_window_policy_1.withAudioSurfaceIsolationHeaders)(
          responseHeaders,
          {
            url: details.url,
...
electron_2.ipcMain.handle('systemCallReadiness:getSnapshot', () =>
  (0, system_call_readiness_1.getSystemCallReadinessSnapshot)()
);
electron_2.ipcMain.handle('systemCallReadiness:refreshSnapshot', () =>
  (0, system_call_readiness_1.refreshSystemCallReadinessSnapshot)()
);
...
electron_2.ipcMain.handle('persistentStore:get', async (_event, key) =>
...
electron_2.ipcMain.handle('miscPersistentStore:get', async (_event, key) =>
  miscPersistentStore.get(key)
);
electron_2.ipcMain.handle(
...
electron_2.ipcMain.handle(
  'walletStorage:write',
  async (_event, fileName, contents) => {
    try {
      const filePath = await getSharedSettingsFilePath(fileName);
      ...
      (0, logger_1.error)(
        `Error in walletStorage:write for "${fileName}"`,
        err
      );
      throw err;
    }
...
electron_2.ipcMain.handle('appSettings:set', async (_event, partial) => {
  ...
  if (!feature_flags_1.isDisabledLegacy) {
    (0, stun_coordinator_1.getStunCoordinator)()?.setLegacyPublicStunFallback(
...
electron_2.ipcMain.handle('hub:getIceServers', async () => {
  if (feature_flags_1.isDisabledLegacy) return [];
...
electron_2.ipcMain.handle(
  ...
  async (_event, urls, success) => {
    if (feature_flags_1.isDisabledLegacy) return { ok: false };
    const c = (0, stun_coordinator_1.getStunCoordinator)();
...
electron_2.ipcMain.handle(
  'file:writeChunk',
  async (_event, filePath, chunk, append) => {
    try {
...
electron_2.ipcMain.handle(
  'cert:ensureForBase',
  async (_event, baseUrl, apiKey) => {
    const result = await (0, local_https_cert_1.ensureCertForBase)(
      baseUrl,
      apiKey
    );
    if (result.success) {
...
async function startDecentralizedStunAfterP2P(network, opts) {
  ...
  const stunPath = (0, path_1.join)(
    (0, path_1.dirname)(chatDb),
    'stun-cache.db'
  );
...
async function ensureReticulumManagersStarted() {
  ...
  if (gcallMgr) {
    ...
    const startedGcallMgr = (0, group_call_1.startGroupCallManager)(
      pm,
      bridgeTransport
    );
  ...
  (0, reticulum_mesh_1.startReticulumMeshCoordinator)(
    (0, reticulum_bridge_1.getReticulumBridge)()
  );
...
electron_2.ipcMain.handle('p2p:start', async (_event, options) => {
  ...
  try {
    ...
    const sharedDbPath = (0, path_1.join)(
      electron_2.app.getPath('appData'),
      'qortal-shared',
      'chat.db'
    );
...
function queuePresenceUpdate(payload) {
  if (
    ...
    (0, logger_1.log)(
      `[Presence] Queueing renderer update for address=${payload.address}`
    );
    queuedPresenceUpdates.set(payload.address, payload);
  } else {
    (0, logger_1.log)(
      '[Presence] Queueing renderer update without address key'
    );
...
async function syncReticulumOverlayStateToBridge(
  ...
  const verifiedPeers = manager.getReticulumVerifiedPeers().map((peer) => ({
...
async function replayReticulumCachedPresence(reason, scheduleFollowup = false) {
  ...
  const address =
    typeof cached.payload?.address === 'string'
...
async function checkReticulumPresenceHealth() {
  if (_1.isQuitting || reticulumHealthRecoveryInFlight) return;
  const manager = (0, presence_1.getPresenceManager)();
  const bridge = (0, reticulum_bridge_1.getReticulumBridge)();
  ...
  (0, logger_1.log)(
    `[ReticulumHealth] Soft replay stale_inbound_ms=${Number.isFinite(inboundAge) ? inboundAge : 'never'} fanout_peers=${health.lastPresenceFanoutPeers ?? 'n/a'} verified=${verifiedCount} active_fanout=${fanoutCount} link_timeouts=${health.recentOverlayLinkTimeouts}`
  );
  ...
  const recoverManagers = () => {
    ...
    if (gcallMgr) {
      ...
      attachGroupCallListeners(
        (0, group_call_1.startGroupCallManager)(pm, currentBridge)
      );
    }
    (0, reticulum_mesh_1.stopReticulumMeshCoordinator)();
...
electron_2.ipcMain.handle(
  'chat:sendTyping',
  async (_event, chatId, authorAddress) => {
    const cm = (0, chat_1.getChatManager)();
    if (!cm) return { success: false, error: 'Chat manager is not running' };
...
electron_2.ipcMain.handle(
  'chat:getHistory',
  async (_event, chatId, limit, beforeTimestamp) => {
    const cm = (0, chat_1.getChatManager)();
    if (!cm) return [];
...
electron_2.ipcMain.handle(
  'chat:markRead',
  async (_event, chatId, upToTimestamp) => {
    const cm = (0, chat_1.getChatManager)();
...
electron_2.ipcMain.handle(
  ...
  async (_event, chatId, eventIds, readerAddress) => {
    const cm = (0, chat_1.getChatManager)();
    if (!cm) return { success: false, error: 'Chat manager is not running' };
    if (
...
electron_2.ipcMain.handle(
  'call:accept',
  async (_event, callId, signature, publicKey, timestamp) => {
    const mgr = (0, call_1.getCallManager)();
    if (!mgr) return { success: false, error: 'Call manager not running' };
...
electron_2.ipcMain.handle(
  'call:reject',
  async (_event, callId, reason, signature, publicKey, timestamp) => {
    const mgr = (0, call_1.getCallManager)();
    if (!mgr) return { success: false, error: 'Call manager not running' };
...
function attachGroupCallListeners(manager) {
  ...
  manager.on('gcall:qortal-group-call-activity', (payload) =>
    broadcastToSet(
...
electron_2.ipcMain.handle(
  'gcall:leave',
  async (_event, roomId, localAddress, signature, publicKey, timestamp) => {
    const mgr = (0, group_call_1.getGroupCallManager)();
    if (!mgr) return { success: false, error: 'GroupCall manager not running' };
...
electron_2.ipcMain.on(
  'gcall:leaveSync',
  (event, roomId, localAddress, signature, publicKey, timestamp) => {
    const mgr = (0, group_call_1.getGroupCallManager)();
    if (!mgr) {
      event.returnValue = {
        success: false,
...
electron_2.ipcMain.handle(
  ...
  async (_event, roomId, topology, signature, publicKey, timestamp) => {
    const mgr = (0, group_call_1.getGroupCallManager)();
    if (!mgr) return { success: false, error: 'GroupCall manager not running' };
...
electron_2.ipcMain.handle(
  ...
  async (_event, roomId, payload, signature) => {
    const mgr = (0, group_call_1.getGroupCallManager)();
    if (!mgr) return { success: false, error: 'GroupCall manager not running' };
...
electron_2.ipcMain.handle(
  'gcall:sendAudio',
  async (_event, roomId, toAddress, data, timing) => {
    const mgr = (0, group_call_1.getGroupCallManager)();
    if (!mgr) return { success: false, error: 'GroupCall manager not running' };
...
electron_2.ipcMain.handle(
  ...
  async (_event, roomId, toAddresses, data, timing) => {
    const mgr = (0, group_call_1.getGroupCallManager)();
    if (!mgr) return { success: false, error: 'GroupCall manager not running' };
    const buf = Buffer.isBuffer(data) ? data : Buffer.from(data);
    attachGroupAudioIpcTiming(buf, timing, {
...
electron_2.ipcMain.handle(
  ...
  async (_event, roomId, toAddresses) => {
    const mgr = (0, group_call_1.getGroupCallManager)();
...
electron_2.ipcMain.handle(
  ...
  async (_event, roomId, address, reason) => {
    const mgr = (0, group_call_1.getGroupCallManager)();
    if (!mgr) return { success: false, error: 'GroupCall manager not running' };
...
electron_2.ipcMain.handle(
  ...
  async (_event, roomId, healthyPeerAddresses) => {
    const mgr = (0, group_call_1.getGroupCallManager)();
    if (!mgr) return { success: false, error: 'GroupCall manager not running' };
...
electron_2.ipcMain.handle('audio-surface:ensure-ready', async (event) => {
  if (!isMainShellSender(event.sender)) {
    (0, logger_1.log)(
      '[GCall:audio-surface] ensure-ready: rejected (not main shell)',
      {
...
electron_2.ipcMain.handle(
  ...
  async (_event, command) => {
    ...
    if (command.type === 'join-group-call' || response.ok === false) {
...
electron_2.ipcMain.on('audio-surface:host-ready', (event) => {
  if (!isAudioSurfaceHostSender(event.sender)) {
    (0, logger_1.warn)(
      '[AudioSurface] rejecting host-ready from unexpected sender',
      {
        senderId: event.sender.id,
      }
    );
...
electron_2.ipcMain.on('gcall:subscribe', (event) => {
  ...
  (0, group_call_1.getGroupCallManager)()?.replayRetainedVerifiedKeyStatesTo(
    event.sender
  );
});
